...
namespace {
  ...
  int minScoreGapless;
  int minScoreGapless2; // added
  size_t numOfSeqs;        //added
...
void alignGapless( SegmentPairPot *gaplessWait ){
  ...
  cout << "# minScoreGapless: " << minScoreGapless << "; minScoreGapless2: " << minScoreGapless2 << endl;
  ...
  for( indexT i = 0; i < query.finishedSize(); i += args.queryStep ){
	...
	// slower overall.
	if( score < minScoreGapless2 ) continue;
	...
	sp.isVisited = false;
	// separate gapless alignments according to which chromosome of reference it aligned to
	whichSeq = text.whichSequence(sp.beg1());
	gaplessWait[whichSeq].add(sp); // add the gapless alignment to the pot
...
bool cmpSegmentPair(const SegmentPair &lhs, const SegmentPair &rhs){
  return lhs.beg2() < rhs.beg2();
}

bool cmpSegmentPair2(const SegmentPair &lhs, const SegmentPair &rhs){
  return lhs.end2() > rhs.end2();
}

// qsort based on the starting position of query
void qsort(SegmentPairPot &gaplessAlns, int lo, int hi){
  std::sort(gaplessAlns.items.begin() + lo, gaplessAlns.items.begin() + hi + 1, cmpSegmentPair);
}

// qsort based on the ending position of query
void qsort2(SegmentPairPot &gaplessAlns, int lo, int hi){
  std::sort(gaplessAlns.items.begin() + lo, gaplessAlns.items.begin() + hi + 1, cmpSegmentPair2);
}

// support alignment on the ride side
void supportRight(SegmentPairPot& gaplessWait, SegmentPairPot& gaplessAlns){
  indexT ref1_end, query1_end, ref2_beg, query2_beg, ref_dist, query_dist;
  size_t i, j;

  // find support alignment for each gaplessWait[i]
  for (i = 0; i < gaplessWait.size() - 1; i++){
    if (gaplessWait.items[i].score >= minScoreGapless){
      gaplessAlns.add(gaplessWait.items[i]);
      continue;
    }

    j = i + 1;
    ref1_end = gaplessWait.items[i].end1();
    query1_end = gaplessWait.items[i].end2();
    ref2_beg = gaplessWait.items[j].beg1();
    query2_beg = gaplessWait.items[j].beg2();

    // search within the distance
    while (query2_beg <= query1_end + (indexT)args.dist){
      if (query2_beg < query1_end || ref2_beg < ref1_end){
        if (j == gaplessWait.size() - 1) break;
        j++;
        ref2_beg = gaplessWait.items[j].beg1();
        query2_beg = gaplessWait.items[j].beg2();
        continue;
      ...
      // if query2_beg >= query1_end && ref2_beg >= ref1_end
      query_dist = query2_beg - query1_end;
      ref_dist = ref2_beg - ref1_end;

      // this alignment is supported by its right side
      if (query_dist <= (indexT)args.dist && ref_dist <= (indexT)args.dist){
        gaplessAlns.add(gaplessWait.items[i]);
        gaplessWait.items[i].isVisited = true; // will not consider its left side
        break;
      }
  ...
  if (gaplessWait.items[gaplessWait.size()-1].score >= minScoreGapless)
    gaplessAlns.add(gaplessWait.items[gaplessWait.size()-1]);
}

// For each alignment with low score that cannot find support on its right side, search its left side
void supportLeft(SegmentPairPot& gaplessWait, SegmentPairPot& gaplessAlns){
  indexT ref1_beg, query1_beg, ref2_end, query2_end, ref_dist, query_dist;
  size_t i, j;

  // find support alignment for each gaplessWait[i]
  for (i = 0; i < gaplessWait.size() - 1; i++){
    if (gaplessWait.items[i].score >= minScoreGapless) continue;
    if (gaplessWait.items[i].isVisited == true) continue;

    j = i + 1;
    ref1_beg = gaplessWait.items[i].beg1();
    query1_beg = gaplessWait.items[i].beg2();
    ref2_end = gaplessWait.items[j].end1();
    query2_end = gaplessWait.items[j].end2();

    // search within the distance
    while (query2_end >= query1_beg - (indexT)args.dist){
      if (query2_end > query1_beg || ref2_end > ref1_beg){
        if (j == gaplessWait.size() - 1) break;
        j++;
        ref2_end = gaplessWait.items[j].end1();
        query2_end = gaplessWait.items[j].end2();
      ...
      // if query1_beg >= query2_end && ref1_beg >= ref2_end
      query_dist = query1_beg - query2_end;
      ref_dist = ref1_beg - ref2_end;
...
void findMoreGaplessAlns(SegmentPairPot *gaplessWait, SegmentPairPot& gaplessAlns){
  size_t i;

  // sort the hits and then find extra alignments
  for (i = 0; i < numOfSeqs; i++){
    if (gaplessWait[i].size() < 1) continue;
    qsort(gaplessWait[i], 0, gaplessWait[i].size()-1);
    supportRight(gaplessWait[i], gaplessAlns);
    qsort2(gaplessWait[i], 0, gaplessWait[i].size()-1);
    supportLeft(gaplessWait[i], gaplessAlns);
  }
}
...
void scan( char strand, std::ostream& out ){
  ...
  SegmentPairPot *gaplessWait;
  gaplessWait = new SegmentPairPot[numOfSeqs];

  alignGapless( gaplessWait );
  findMoreGaplessAlns( gaplessWait, gaplessAlns );

  // we just want gapless alignments
  if( args.outputType == 1 ){
    size_t i;
    for (i = 0; i < gaplessAlns.size(); i++){
      Alignment aln;
      aln.fromSegmentPair(gaplessAlns.items[i]);
      aln.write( text, query, strand, args.isTranslated(), alph, evaluer, args.outputFormat, out );
    }
    return;
  }
...
void readVolume( unsigned volumeNumber ){
  ...
  minScoreGapless2 = (int) floor(minScoreGapless * args.gaplessRatio); // added
